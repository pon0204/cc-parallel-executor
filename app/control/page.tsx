  ...
  const createSession = async (prompt: string, taskId?: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        alert(`セッション作成失敗: ${errorData.error || 'Unknown error'}`);
        return;
      }
      ...
      if (!data.sessionId) {
        console.error('セッションIDが返されませんでした:', data);
        alert('セッションIDが返されませんでした');
        return;
      }
                ...
                <Input
                  ...
                  onKeyPress={async (e) => {
                    if (e.key === 'Enter' && customPrompt) {
                      const sessionId = await createSession(customPrompt);
                      if (sessionId) {
                        setCustomPrompt('');
                      }
                    }
